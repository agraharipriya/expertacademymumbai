      ...
      <div className="max-w-6xl mx-auto">
        ...
        <div className="grid grid-cols-2 gap-4 max-w-4xl mx-auto">
          {images.slice(0, 4).map((url, i) => (
            <div
              key={i}
              className="rounded-2xl overflow-hidden shadow-sm hover:shadow-lg transition-all hover:scale-[1.02] bg-white"
            >
              <img 
                src={url} 
                alt={labels[i]} 
                ...
                onError={(e) => {
                  ...
                  e.target.parentElement.innerHTML = `<div class="w-full  flex items-center justify-center bg-gray-100 text-gray-400">📷 ${labels[i]}</div>`;
                }}
              />
            </div>
          ))}
